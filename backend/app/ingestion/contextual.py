...
Every chunk gets a deterministic prefix (regulation, article, heading) —
that alone disambiguates most statutory chunks. The optional LLM pass adds a
one-sentence semantic summary of how the chunk relates to its article, which
...
from app.ingestion.chunker import ArticleChunk

CONTEXT_PROMPT = (
    "You situate legal text chunks for a retrieval system. In ONE sentence, state what this "
    "chunk of {regulation_title}, {article_ref} ({heading}) covers, so it can be found by "
    "paraphrased queries. Reply with the sentence only.\n\nCHUNK:\n{content}"
)


def deterministic_prefix(regulation_title: str, chunk: ArticleChunk) -> str:
    heading = f" — {chunk.heading}" if chunk.heading else ""
    return f"{regulation_title}, {chunk.article_ref}{heading}."
...
async def llm_prefixes(
    ...
    async def describe(chunk: ArticleChunk) -> str:
