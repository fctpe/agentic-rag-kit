...
1. **Ingestion (offline)** — `app/ingestion/pipeline.py` parses the EU AI Act and GDPR into citable units — articles (`Art. 6`) and annexes (`Annex III`), each read from its own EUR-Lex container id (`art_6`, `anx_III`) — either from live EUR-Lex or, with `--source fixture`, from the committed `data/fixtures/`. Both paths chunk on unit boundaries (≤700 tokens) and store everything in one Postgres; they differ in exactly one way, deliberately. `--source fixture` *reads* the LLM context sentence and the embedding vector from the fixtures, so it calls no provider at all and `make corpus-digest` can prove it reproduces; `--source network` generates and requests them, and neither call is deterministic. Annexes are chunked exactly like articles — ingesting Annex III whole would let it crowd out the articles that cite it. All three decisions, with the measurements behind them: [ADR 0003](adr/0003-structural-chunking-contextual-prefixes.md).
