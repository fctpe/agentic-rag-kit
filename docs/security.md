...
| LLM02 Sensitive information disclosure | Structural PII redaction (`app/security/redaction.py`) runs in `guard_input`, **before** text can reach the model, the checkpointer, or a trace. Redaction events land in the audit log without the raw values, and spans and log lines carry counts, ids, model names and verdicts only — never prompts, queries or retrieved text (ADR 0006). Failures are held to the same rule: `error_fields` puts the exception *type* and its frames on the span and in the log and drops `str(err)`, because a SQLAlchemy `DBAPIError` message embeds the bound parameters and the query is one of them. |
